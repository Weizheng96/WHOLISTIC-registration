function motion_current=getMotion_Wei_v21d1(dat_mov,dat_ref,smoothPenalty_raw,option)
%% v21: based on v20, new penalty term
...
for layer = layer_num:-1:0
    ...
    if isfield(option,'motion') && ~isempty(option.motion)
        ...
        motion_ref(:,:,:,1)=imresize3(option.motion(:,:,:,1),[x,y,z])/(SZ(1)/x);
        motion_ref(:,:,:,2)=imresize3(option.motion(:,:,:,2),[x,y,z])/(SZ(2)/y);
        motion_ref(:,:,:,3)=imresize3(option.motion(:,:,:,3),[x,y,z])/(SZ(3)/z);
    else
        motion_ref = gpuArray(zeros(x,y,z,3));
    end
    %% initilize the motion of each layer
    if layer == layer_num
        motion_current=motion_ref;
    else
