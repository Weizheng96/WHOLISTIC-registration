function phi_current=getMotion_Wei_v4(dat_mov,dat_ref,smoothPenalty)
%% v4: add penalty
...
%         neiSum=smoothPenalty*getNeiSum(phi_current);
%         smoothPenaltySum=smoothPenalty*sum(AverageFilter,'all');

        stepFactor=min((iter-1)/(iterNum-2),1);
        neiSum=smoothPenalty*stepFactor*getNeiSum(phi_current);
        smoothPenaltySum=smoothPenalty*stepFactor*sum(AverageFilter,'all');
    
        phi_gradient=getFlow3_withPenalty(Ixx,Ixy,Ixz,Iyy,Iyz,Izz,Ixt,Iyt,Izt,smoothPenaltySum,neiSum);
