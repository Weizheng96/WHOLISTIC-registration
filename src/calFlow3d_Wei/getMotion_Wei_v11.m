function phi_current=getMotion_Wei_v11(dat_mov,dat_ref,smoothPenalty_raw)
%% v11: only deformable
...
%     oldPenaltyError=inf;
    for iter = 1:iterNum
        ...
        pos_diff=max((phi_current(2:end,:,:,1)-phi_current(1:end-1,:,:,1)+1)/2-0.05,0);
        movRange_pos(:,:,:,1)=cat(1,pos_diff,zeros(1,y,z));
        movRange_neg(:,:,:,1)=cat(1,zeros(1,y,z),-pos_diff);
        pos_diff=max((phi_current(:,2:end,:,1)-phi_current(:,1:end-1,:,1)+1)/2-0.05,0);
        movRange_pos(:,:,:,2)=cat(2,pos_diff,zeros(x,1,z));
        movRange_neg(:,:,:,2)=cat(2,zeros(x,1,z),-pos_diff);
        pos_diff=max((phi_current(:,:,2:end,1)-phi_current(:,:,1:end-1,1)+1)/2-0.05,0).*0;
        movRange_pos(:,:,:,3)=cat(3,pos_diff,zeros(x,y,1));
        movRange_neg(:,:,:,3)=cat(3,zeros(x,y,1),-pos_diff);
