function motion_current=getMotion_Wei_v19d5(dat_mov,dat_ref,smoothPenalty_raw,option)
%% v19d3: based on v19d3, new eary stop
...
%     fprintf("\nDownsample:"+layer+"\n");
    for iter = 1:iterNum
        ...
        %%
        motion_current_temp = motion_current+motion_update;
        %% calculate error (make it slow)
        data1_corrected=gather(correctMotion_Wei_v2(data1,motion_current_temp));
        diffError=gather(mean((data2-data1_corrected).^2,'all'));
        penaltyRaw=getNeiDiff(motion_current_temp(xG,yG,zG,:),1);
        ...
        Error=diffError+penaltyError;
        fprintf("Downsample:"+layer+"\tIter:"+iter+"\tError:\t"+Error+"\tDiff:\t"+diffError+"\n");
        ...
        %% update the current motion and momentum
        motion_current = motion_current_temp;
