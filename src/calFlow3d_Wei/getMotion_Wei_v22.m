function motion_current=getMotion_Wei_v22(dat_mov,dat_ref,smoothPenalty_raw,option)
%% v22: based on v21, independent refference motion
...
for layer = layer_num:-1:0
    ...
    %% get motion_ref
    if isfield(option,'motion_ref') && ~isempty(option.motion_ref)
        motion_ref = gpuArray(zeros(x,y,z,3));
        motion_ref(:,:,:,1)=imresize3(option.motion_ref(:,:,:,1),[x,y,z])/(SZ(1)/x);
        motion_ref(:,:,:,2)=imresize3(option.motion_ref(:,:,:,2),[x,y,z])/(SZ(2)/y);
        motion_ref(:,:,:,3)=imresize3(option.motion_ref(:,:,:,3),[x,y,z])/(SZ(3)/z);
    else
        motion_ref = gpuArray(zeros(x,y,z,3));
    end
...
%     fprintf("\nDownsample:"+layer+"\n");
    for iter = 1:iterNum
        ...
        neiDiff=getNeiDiff(motion_current(xG,yG,zG,:)-motion_ref(xG,yG,zG,:),1);
