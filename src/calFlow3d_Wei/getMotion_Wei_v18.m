function motion_current=getMotion_Wei_v18(dat_mov,dat_ref,smoothPenalty_raw,option)
...
%     fprintf("\nDownsample:"+layer+"\n");
    for iter = 1:iterNum
        ...
        %% get motion update of control points based on data term
        ...
        motion_update=getFlow3_NoPenalty4(Ixx,Ixy,Ixz,Iyy,Iyz,Izz,Ixt,Iyt,Izt);
        ...
        %% get motion update of control points based on spatial term
        patchConnectNum=(r*2+1)^2;
        smoothPenaltySum=smoothPenalty*patchConnectNum;
        neiDiff=smoothPenaltySum*getNeiDiff(motion_current(xG,yG,zG,:),1);
        motion_update=getFlow3_onlyPenalty4(Ixx,Ixy,Ixz,Iyy,Iyz,Izz,smoothPenaltySum,neiDiff,zRatio);
        %% get all pixels' the motion update based on control points' motion update
        x_new = (x_ind-r-1)/(2*r+1)+1;
        x_new = min(max(x_new,1),size(motion_update,1));
        y_new = (y_ind-r-1)/(2*r+1)+1;
