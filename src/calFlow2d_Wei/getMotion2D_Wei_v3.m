function motion_current=getMotion2D_Wei_v3(dat_mov,dat_ref,smoothPenalty_raw,option)
%% v3: based on v2, single
...
for layer = layer_num:-1:0
    ...
    mask_mov=single(imresize3(option.mask_mov,[x,y,z]));
    %% initial old error
    oldError=inf(3,1);
    %% penalty parameters
...
%         fprintf("Downsample:"+layer+"\tIter:"+iter+"\tError:\t"+currentError+"\tDiff:\t"+diffError+"\n");
        ...
        [Ix,Iy,~]=getSpatialGradientInOrg_Wei(data1,motion_current);
        Ix(mask)=0;Iy(mask)=0;
%         Iz(mask)=0;
...
%         Izt = Izt(xG,yG,zG);
        ...
        motion_update_normalized=getFlow2D_withPenalty_v2(Ixx,Ixy,Iyy,Ixt,Iyt,smoothPenaltySum,neiSum);
