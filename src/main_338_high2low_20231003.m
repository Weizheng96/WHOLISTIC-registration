...
resPathName="/work/public/Virginia Rutten/221124_f338_ubi_gCaMP7f_bactin_mCherry_CAAX_8505_7dpf_hypoxia/" + ...
    "f338_High2Low_r20_rp1_20231018";
%% parameters
option.layer=3;
option.iter=100;
option.r=12;
smoothPenaltyTemplate=1;
thresFactor=5;
smFactor=50;
padSize=0;%298
%% generate the folder
...
[dat_ana_corrected2,option.zRatio,dat_mov_Ca]=correctAnaFile_v2(anaFilePath,resPathName); % saved as "ref_ch2_corrected.ome.tiff"
%% normalize anatomical
dat_mov=dat_ana_corrected2(1+padSize:end-padSize,:,:);
...
tifwrite(uint16(temp),fullfile(resPathName,"data_corrected_originalSize.tif"));

temp=correctMotion_Wei_v3(dat_mov_Ca,x_new,y_new,z_new);
tifwrite(uint16(temp),fullfile(resPathName,"data_corrected_Ca_originalSize.tif"));

save(fullfile(resPathName,"data.mat"),"x_new","y_new","z_new","dat_mov")
%%


% tifwrite(uint16(cat(2,dat_mov(:,:,round(option.zLst)),dat_out,dat_ref)),...
%     fullfile(resPathName,"raw_corrected_template.tif"))
% 
% tifwrite(uint16(dat_mov(:,:,round(option.zLst))),...
%     fullfile(resPathName,"raw.tif"))
% 
% tifwrite(uint16(dat_ref),...
%     fullfile(resPathName,"template.tif"))

% dat_anno=tifread(AnnoFilePath);
% dat_anno=dat_anno(1+padSize:end-padSize,:,:);
